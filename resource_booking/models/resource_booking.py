...
class ResourceBooking(models.Model):
    ...
    def _compute_duration(self):
        ...
        for one in self:
            ...
            if not one.id:
                one.duration = self.default_get(["duration"]).get(
                    "duration", one.type_id.duration
                )
            # Get duration from type only when changing type or when creating from ORM
            elif not one.duration or one._origin.type_id != one.type_id:
                one.duration = one.type_id.duration
            # Get it from meeting only when available
            elif one.meeting_id:
                one.duration = one.meeting_id.duration
    ...
    def _compute_stop(self):
        ...
        for one in self:
            try:
                one.stop = one.start + timedelta(hours=one.duration)
            except TypeError:
